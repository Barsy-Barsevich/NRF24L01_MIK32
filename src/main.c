 ...
 * CS   - 1.6
 * CE   - 1.7 */
#define NRF_CE_PORT     GPIO_1
#define NRF_CE_PIN      GPIO_PIN_7
#define NRF_CS          SPI_CS_2
 ...
 * CS   - 0.4
 * CE   - 2.7  */
#define NRF_CE_PORT     GPIO_2
#define NRF_CE_PIN      GPIO_PIN_7
#define NRF_CS          SPI_CS_0
#define NRF_SPI         SPI_0
#endif
/*---------------------------------------------------------------------*/
...
int main()
{
    ...
    while (1)
    {
        while (NRF24L01_RX_data_ready(&nrf) != HAL_OK)
        {
            xprintf(".");
            // uint8_t buf1[32];
            // uint8_t dt_reg = NRF24L01_ReadReg(&nrf, NRF_CONFIG);
    ...
    {
        HAL_DelayMs(500);
        sprintf(buffer, "Packet received, time %lu", HAL_Millis());
...
{
    nrf.interface.ce_pin = NRF_CE_PIN;
    nrf.interface.ce_port = NRF_CE_PORT;
    nrf.interface.spi = &hspi0;
    nrf.interface.cs = NRF_CS;
#ifdef RECEIVE
